...
<div class="row">
    ...
    <div class="col-md-8" style="text-align: right;">
        ...
        <a href="<?php print base_url() . '/tmp/report_template/ops_monthly.xls' ;?>" target="_self"><button type="button" name="template_btn" id="template_btn" class="btn btn-default btn-md"><i class="fa fa-file-text"></i> &nbsp; <?php print $this->lang->line('template'); ?></button></a>
        &nbsp; &nbsp; &nbsp;
...
jQuery(document).ready(function($){
    ...
    var uploader = $("#uploader").plupload({
        ...
        url : "<?php print base_url('adminpanel/ops_monthly/import_report'); ?>",
...
$("#pending").click(function() {
    ...
    paging = {
        ...
        order_by : 'import_date',
        sort_order : 'desc',
        ajaxUrl: '<?php print base_url('adminpanel/ops_monthly/get_pending'); ?>',
        search_data : {
            status: 0,
...
var confirmImport = function () {
    ...
    $.ajax({
        url: '<?php print base_url('adminpanel/ops_monthly/confirm_pending'); ?>',
        data: requestData,
        type: 'post',
        dataType: 'json',
...
$('#delete_import').click(function() {
    bootbox.confirm('<?php print $this->lang->line('msg_delete_import'); ?>', function(confirmed){
        if (confirmed) {
            ...
            $.ajax({
                url: '<?php print base_url('adminpanel/ops_monthly/delete_pending'); ?>',
                data: requestData,
                type: 'post',
                dataType: 'json',
                success: function(data) {
...
var deleteData = function(data_id) {
    bootbox.confirm('Are you sure to delete this record?', function(confirmed){
        if (confirmed) {
            $.ajax({
                url: "<?php print base_url('adminpanel/ops_monthly/delete_report'); ?>",
...
var drawTable = function (data) {
    ...
    if ( paging.ajaxUrl == '<?php print base_url('adminpanel/ops_monthly/get_report'); ?>' ) {
    ...
    $.each( data, function( key, value ) {
        ...
        <?php if( isset( $permission['delete'] ) ): ?>
        html +='<td style="white-space: nowrap;"><a href="#" onclick="deleteData(' + value['ops_monthly_id'] + ')" class="btn btn-danger btn-circle" title="<?php print $this->lang->line('delete')?>" data-toggle="tooltip" data-placement="top" data-original-title="" data-method="DELETE"><i class="fa fa-trash"></i></a></td>';
        <?php endif; ?>
