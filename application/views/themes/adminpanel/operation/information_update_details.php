...
<?php if ($add) { ?>
    <div class="col-xs-12" style="text-align: right; margin-bottom: 20px;">
        <a href="<?php print base_url() . "adminpanel/operation/information_update_details_insert/" . $report->shift_reports_id; ?>">
            <button type="button" class="btn btn-primary">
                <span><i class="fa fa-plus pd-r-5"></i>&nbsp;<?php echo $this->lang->line('add_new'); ?></span>
            </button>
        </a>
        <a href="<?php print base_url('adminpanel/operation/information_update') . "?type=session"; ?>">
            <button type="button" class="btn btn-primary">
        ...
        <div class="panel-body table-responsive">
            <table class="table ">
                ...
                <tr>
                    ...
                    <td colspan="3"><span class="info" style="word-break: break-all;"><?php print $report->remarks ?></span></td>
                </tr>
            </table>
        </div>
    </div>
...
<div class="row margin-top-30">
    <div class="col-xs-12">
        ...
        <div class="table-responsive" id="information_update_details_table">
...
<script type="text/javascript">
    ...
    var paging = {
        ...
        search_data: {},
        permission: {},
        ajaxUrl: "<?php print base_url('adminpanel/operation/get_information_update_details') . '/' . $report->shift_reports_id; ?>"
    };
    ...
    var deleteData = function (id) {
        bootbox.confirm(delete_message, function (confirmed) {
            if (confirmed) {
                $.ajax({
                    url: "/adminpanel/operation/information_update_details_delete/" + id,
                    type: "post",
    ...
    var drawTable = function (data) {
        ...
        if (data.length != 0) {
            $('#information_update_details_table').css('display', 'block');
            $('#no_result').css('display', 'none');
        } else {
            $('#information_update_details_table').css('display', 'none');
        ...
        $.each(data, function (key, value) {
            ...
            if (paging.permission['edit']) {
                html += '<a href="<?php print base_url('adminpanel/operation/information_update_details_edit') . '/' . $report->shift_reports_id . '/';?>' + value['follow_up_id'] + '" class="btn btn-primary btn-circle edit" title="<?php print $this->lang->line('edit')?>" data-toggle="tooltip" data-placement="top" data-original-title="Edit"><i class="fa fa-pencil-square"></i></a>';
            }
