...
class Log_in_out extends Admin_Controller {
    ...
    public function __construct() {
        ...
        $this->permission = self::check_action(11);
        $this->load->model('adminpanel/Performance_log_in_out_model');
    ...
    public function index() {
        ...
        $this->quick_page_setup(Settings_model::$db_config['adminpanel_theme'], 'adminpanel', $this->lang->line('log_in_out'), 'performance_report/log_in_out', 'header', 'footer', '', $content_data);
    }
    ...
    public function export_report() {
        ...
        $resultObj = $this->Performance_log_in_out_model->get_records( 0, 0, $order_by, $sort_order, $search_data, 1, $this->my_permission->find_permission() );
        $total_rows = $this->Performance_log_in_out_model->count_confirm_records($search_data, $this->my_permission->find_permission() );
    ...
    public function get_pending() {
        if ( $this->input->post() ) {
            ...
            $per_page = Settings_model::$db_config['members_per_page'];
            $resultObj = $this->Performance_log_in_out_model->get_records($per_page, $offset, $order_by, $sort_order, array(), 0, array() );
            $content_data['total_rows'] = $this->Performance_log_in_out_model->count_pending_records();
    ...
    public function import_report() {
        ...
        $pendingNo = $this->Performance_log_in_out_model->count_not_my_pending();
    ...
    public function delete_report() {
        ...
        if ( $this->input->post() ) {
            ...
            $recordDetails = $this->Performance_log_in_out_model->get_single_record( $id )->result('array');
    ...
    public function confirm_pending() {
        ...
        if ( $this->input->post() ) {
            $array = $this->Performance_log_in_out_model->get_pending_import_by();

            if ( in_array( $array[0]['import_by'], $this->my_permission->find_permission() ) ) {
                try {
                    $this->Performance_log_in_out_model->confirm_records_import();
    ...
    public function delete_pending() {
        if($this->permission->delete != 'yes') {  // check permission
            ...
            if ( $this->input->post() ) {
                ...
                if ( in_array( $array[0]['import_by'], $this->my_permission->find_permission() ) ) {
                    try {
                        $this->Performance_log_in_out_model->delete_pending_records();
                        echo json_encode( array('error' => false, 'message' => $this->lang->line('delete_success') ),true );
                    } catch ( Exception $e ){
                        echo json_encode( array( 'error' => true, 'message' => $this->lang->line('account_unknown_error') ), true ); 
                    }
