...
class Settings_model extends CI_Model {
    ...
    protected function _load_settings() {
        ...
        if (empty($data)) {
            ...
            recaptchav2_secret, oauth2_enabled,footer_title,system_role,predefined_email,logo,data_mask, search_section,site_language,operator_leader_role');
            ...
            if($query->num_rows() == 1) {
                ...
                self::$db_config['logo'] = $row->logo;
                self::$db_config['data_mask'] = $row->data_mask;
                self::$db_config['search_section'] = $row->search_section;
                self::$db_config['site_language'] = $row->site_language;
                self::$db_config['operator_leader_role'] = $row->operator_leader_role;
            ...
            self::$db_config['logo'] = $data['logo'];
            self::$db_config['data_mask'] = $data['data_mask'];
            self::$db_config['search_section'] = $data['search_section'];
            self::$db_config['site_language'] = $data['site_language'];
            self::$db_config['operator_leader_role'] = $data['operator_leader_role'];
        }
    }
